...
1. Generate localization key from JSONPointer token; see `gamedata_localize_key`
...
function localize!(jws::JSONWorksheet, filename, keycolumn::AbstractString)
    ...
    for (token, text) in localize_targets
        if isa(keycolumn, JSONPointer.Pointer)
            ...
            finalkey = gamedata_localize_key(token, keyvalues)
        else
            # uses rownumber
            finalkey = gamedata_localize_key(token)
        end
        if haskey(localizedata, finalkey)
            throw(ArgumentError("`$finalkey` is duplicated. Please check if keycolumn in config.json is unique\n$(keycolumn) "))
        end
...
"""
    gamedata_localize_key(tokens)
    gamedata_localize_key(tokens, keyvalues)

generate localization key from JSONPointer token
"""
function gamedata_localize_key(tokens)
...
function gamedata_localize_key(tokens, keyvalues)
    # $gamedata.(FileName)#/(JSONPointer)/keycolum_values"
    idx = ""
    for el in keyvalues
        if !ismissing(el) && !isnothing(el) && !isempty(el)
    ...
    gamedata_localize_key(tokens, idx)
end
function gamedata_localize_key(tokens, combinedkey::AbstractString)
